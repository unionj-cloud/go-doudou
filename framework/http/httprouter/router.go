...
func (r *Router) saveMatchedRoutePath(path string, handle Handle, name ...string) Handle {
	return func(w http.ResponseWriter, req *http.Request, ps Params) {
		if ps == nil {
			...
			if size > 1 {
				ps[1] = Param{Key: MatchedRouteNameParam, Value: name[0]}
			}
			handle(w, req, ps)
			r.putParams(psp)
		} else {
			...
			if len(name) > 0 {
				ps = append(ps, Param{Key: MatchedRouteNameParam, Value: name[0]})
			}
			handle(w, req, ps)
		}
	}
...
func (r *Router) Handle(method, path string, handle Handle, name ...string) {
	...
	if r.SaveMatchedRoutePath {
		...
		handle = r.saveMatchedRoutePath(path, handle, name...)
	}
