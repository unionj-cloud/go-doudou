...
import (
	...
	"github.com/unionj-cloud/go-doudou/toolkit/net"
...
func (p *Process) times1() (cpu.TimesStat, error) {
	_, _, cpuTimes, _, _, _, _, err := p.fillFromStatWithContext(context.Background())
	if err != nil {
...
func (p *Process) fillFromTIDStatWithContext(ctx context.Context, tid int32) (uint64, int32, cpu.TimesStat, int64, uint32, int32, PageFaultsStat, error) {
	...
	contents, err := ioutil.ReadFile(statPath)
	if err != nil {
		return 0, 0, cpu.TimesStat{}, 0, 0, 0, PageFaultsStat{}, err
	}
